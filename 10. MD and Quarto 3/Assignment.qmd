...
viewers_start <- str_sub(season_data$viewers)[1]
viewers_end <- str_sub(season_data$viewers)[episodes_no]
...
Season `r params$season` of *Game of Thrones* consisted of `r episodes_no` episodes that aired between `r show_start` and `r show_end` on HBO. The show gathered an average of `r avg_viewers` first-day TV viewers in the US, with the number `r change` from `r viewers_start` to `r viewers_end` million by the end of the season. It had `r nrow(season_data)` episodes. 
